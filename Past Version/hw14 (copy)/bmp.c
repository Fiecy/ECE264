...
bool write_bmp(FILE* fp, BMPImage* image, char** error) {
	...
	//check_header = fwrite(&(image->header), 1, image->header.offset, fp);
	if (check_header != 1) {
		return false;
	}
	check_data = fwrite(image->data, (image->header.size - image->header.offset), 1, fp);
	//check_data = fwrite(image->data, 1, (image->header.size - image->header.offset), fp);
	if (check_data != 1) {
		return false;
	}
	return true;
...
BMPImage* crop_bmp(BMPImage* image, int x, int y, int w, int h, char** error) {
	...
	long int starting_point_calc = (((image->header.height_px - y - h) * image->header.width_px) + x) * byte_per_px + (image->header.height_px - y - h) * ((4 - ((image->header.width_px * byte_per_px) % 4))%4); // calculate the location of first pixel
