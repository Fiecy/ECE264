...
int main(int argc, char *argv[]) {
	...
	struct Person* detach1_person = detach_person(&team, "Liangts");
	fprintf(stdout,"detached %s, favorite %s\n",detach1_person->name, detach1_person->favorite->name);
	fprint_team(&team, stdout);
			
	struct Person* add1_person = add_person(&team, "YFSB", "Momo");
	fprintf(stdout,"add person = %s favorite = %s\n", add1_person->name, add1_person->favorite->name);
	fprint_team(&team, stdout);
	fprintf(stdout, "team->head = %s, team->tail = %s\n", team.head->value->name, team.tail->value->name);
	
	struct Person* the1_person = find_person(&team, "YL");
	fprintf(stdout,"find_person = %s, favorite = %s\n",the1_person->name, the1_person->favorite->name);
	fprint_team(&team, stdout);

	struct Person* detach1_person2 = detach_person(&team, "Not in team");
	assert(detach1_person2 == NULL);
	fprintf(stdout,"detach a person not in team return NULL\n");

	struct Person* the1_person2 = find_person(&team, "Not in team");
	assert(the1_person2 == NULL);
	...
	struct Person* add3_1_person = add_person(&team3_1, "YFSB", "Anna");
	fprintf(stdout,"add person = %s favorite = %s\n", add3_1_person->name, add3_1_person->favorite->name);
	fprint_team(&team3_1, stdout);
	fprintf(stdout, "team->head = %s, team->tail = %s\n", team3_1.head->value->name, team3_1.tail->value->name);
	struct Person* add3_1_person2 = add_person(&team3_1, "Hex", "YFSB");
	fprintf(stdout,"add person = %s favorite = %s\n", add3_1_person2->name, add3_1_person2->favorite->name);
	fprint_team(&team3_1, stdout);
	fprintf(stdout, "team->head = %s, team->tail = %s\n", team3_1.head->value->name, team3_1.tail->value->name);
